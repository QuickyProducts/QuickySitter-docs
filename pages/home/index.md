...
- **LinkSet Data storage**: pose data, channel settings and the prop database live in LSD instead of script memory, so complex furniture stays stable past Mono's 64 KB cap. The practical limit is the linkset's shared **128 KiB LSD pool** (poses, props, settings and personal offsets all draw from it): room for roughly **1,700 poses** with typical entry sizes. The pool is per furniture, not per slot, so a multi-sitter holds far more than stock AVsitter's ~200 poses per script-heap ever could.
