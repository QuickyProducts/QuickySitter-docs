...
| `/5 cleanup` | Build finalization: removes `[QS]adjuster` and the `[AV]helper` object from the prim (mirrors the stock AVsitter cleanup step), and broadcasts a finalize signal that creator-only plugins answer by removing themselves — with the QuickySitter Pro Animesh plugin present, the whole animesh kit (dummy bodies, `animeshconfig`, both scripts, rezzed dummies) goes too, no extra `[FINALIZE]` needed. Removing the adjuster is what switches authoring off: without it no `[HELPER]`/`[HELPER HUD]`/`[NEW]`/`[DUMP]`/`[SAVE]` exists on the piece, while personal pose offsets keep working. |
