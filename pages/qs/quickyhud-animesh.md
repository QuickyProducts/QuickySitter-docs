...
3. Pick a seat, then pick a body from the numbered list — the dummy rezzes on that seat and starts the partner animation.
...
- Name it `[QS]dummyYourLabel` and drop it into the furniture — it is discovered automatically. The label must not contain a `|` character.
- Keep its own name and declare it in the **`animeshconfig`** notecard: `ANIMESH <object name> | <description>` (one per line, up to 10 entries, description up to 48 characters). The notecard is only for your own extras — it is never overwritten by updates.
...
- Swapping an avatar **with** a dummy (trading places) is not supported — the dummy simply yields.
...
`[FINALIZE]` in the seat list (owner-only, with a confirm dialog) strips the complete animesh kit off the piece: all `[QS]dummy` bodies, the `animeshconfig` notecard and both plugin scripts. Run it on a finished piece **before you sell it** — the customer copy then carries no trace of the tool.
...
- Left-click selection often misses animesh — use right-click → Touch.
