...
**Entering it.** The owner clicks `[QUICKYHUD]` in the adjust menu (or flips ADJUSTMODE from the HUD's own settings). sitB sends 90266 `"On"` to hudproxy, and entering ADJUSTMODE **auto-attaches a HUD to the seated operator** (`ATTACH_FOR_ADJUST` 90274 → hudadmin), so there is no rummaging in inventory. The pose menu gains a `[DONE]` exit button.
...
- **Quiet (ADJUSTMODE path):** only the settings-service URL and the `COPY ABOVE/BELOW` banners are shouted to the owner. No per-line chat, because the URL is the deliverable.
