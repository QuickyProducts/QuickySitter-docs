...
- **Feature**: The HUD's arrow controls are now also available as a menu, for sitters who are not wearing a HUD. Drop the new optional `[QS]huddialog` script into the furniture next to the other Quicky scripts and the `[POSE]` entry in the `[ADJUST]` menu opens an arrow pad instead of the classic dialog: the same camera-relative directions as the HUD, three step sizes, a Move and a Rotate page, and every press is saved right away. Each sitter gets their own pad, so several people can adjust at the same time. Without the script nothing changes. Needs the matching QuickySitter update (`[QS]sitB`) and the QuickyHUD scripts in the furniture.
