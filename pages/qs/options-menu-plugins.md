...
- **`[OPTIONS]` menu only.** The pose menu's main button strip (`[ADJUST]`, `[NEW]`, `[DUMP]`) and the `[ADJUST]` submenu are not Plugin-Registry targets in v1. Plugin authors who want buttons there must still patch sitB directly, or use the legacy notecard `ADJUST` line, which is functionally identical to a registry entry but boot-time and scoped to the ADJUST submenu (see [Adjustment Workflow](adjustment-workflow.html#helper-menu-entries)).
