...
- **Feature**: New `hudconfig` attach mode `menuplus`. It works like `menu`, no auto-attach, but the HUD adds its own `💠 POSE HUD` entry to the furniture's `[ADJUST]` menu, so the button no longer has to be written into the `AVpos` notecard by hand and can no longer be forgotten. Switching the mode back removes the entry again. `menu` and `auto` are unchanged, so existing furniture behaves exactly as before.
