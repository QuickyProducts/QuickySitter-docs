...
- **Feature**: In "menu" attach mode the HUD now puts its own `💠 POSE HUD` entry into the furniture's `[ADJUST]` menu. Setting the attach mode is enough, the button no longer has to be added to the `AVpos` notecard by hand, and forgetting that line can no longer leave a piece where the HUD has no way to arrive. Switching the mode back to "auto" removes the entry again. Furniture that already carries a hand-made button keeps it and simply shows both.
