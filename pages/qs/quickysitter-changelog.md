...
- **Feature**: Prop scale & worn-fit support: props equipped with the `[QS]objectadjust` script (included with QuickySitter — drop it into your prop next to `[AV]object`) can be resized in the editor, or fitted directly on the body for attachment props, and saved with `[SAVE]` — no more take-back-and-replace loop. Owners can also fine-tune a rezzed prop's size by touch (±1/5/10 % menu, `[RESTORE]`).
