...
| `[QS]root-security` | Sit/menu access gate (ALL / OWNER / GROUP); sends LinkMsg 90202 to sitA. Since 1.25 also a third **Adjust** ACL (OWNER / GROUP / ALL, default OWNER) set from the `[SECURITY]` menu and published to the `qs:sec:adjust` LSD key; `[QS]sitB` / `[QS]adjuster` read it to gate who may enter the adjust workflows (`[HELPER]` / `[HELPER HUD]` and owner-gated registered `[ADJUST]` entries). The `[SECURITY]` menu shows the Adjust category only while `[QS]adjuster` is present (`qs:alive:adjuster`), and the level can also be set with the owner chat shortcut [`/5 adjust`](chat-commands.html), which works even without this plugin. Resets to OWNER on `CHANGED_OWNER` (both writers). |
