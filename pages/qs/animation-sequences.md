...
Step directives: `PLAY <pose>` (play a pose), `WAIT <seconds>` (hold, float), `SAY`/`WHISPER <text>` (chat), `SOUND <name>|<volume>` (play a sound; **field 2 is the volume**, 0.0 to 1.0, not a loop flag), and `LOOP` (jump back to the sequence's first step at the end). Without `LOOP`, the sequence stops after the last step and the final pose keeps looping.
