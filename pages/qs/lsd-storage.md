...
All keys are namespaced `qs:*`. `<ch>` is the sitter slot (0-based, matches `SCRIPT_CHANNEL` in sitA/sitB).
...
The `QSO:*` namespace is intentionally outside the `qs:*` family — it lives outside the seed-and-forget layout, managed lazily across script lifetimes by `[QS]offset`. It also shares the prim with QuickyHUD's protected `QPP_CFG:*` keys without colliding.

## Why pose defaults moved to LSD

Stock AVsitter holds *everything* in `[AV]sitB`'s script memory: `DATA_LIST` (pose-anim mappings) and `POS_ROT_LIST` (positions/rotations). On `[HELPER] [SAVE]`, stock updates only those in-memory lists — the AVpos notecard is **not** auto-written. The `[DUMP]` button exists exactly for that reason: the creator copy-pastes the dump output back into the notecard manually, or loses unsaved changes on the next script reset.
...
The split exists because LSD has a write throttle and a hard size cap. We don't want personal offsets to crowd out pose defaults when storage gets tight. The RAM tier is the safety valve.
...
| Trigger | Effect |
|---------|--------|
| Notecard changed (`CHANGED_INVENTORY` in boot) | boot resets, re-parses notecard, **rewrites** `qs:cfg`/`qs:sitter`/`qs:p:*`/`qs:meta:*`, then resets every sitA and sitB so they bootstrap from fresh LSD |
...
Per-sitter playback (`MY_SITTER`, `CONTROLLER`, `MY_CUSTOMS`, `DEFAULT_POSITION/ROTATION`, `CURRENT_POSITION/ROTATION`, `CURRENT_POSE_NAME`, `CURRENT_ANIMATION_FILENAME`, …); first-sit defaults; gender variants; settings cache read from `qs:cfg`; sitter list; dialog state.

### `[QS]sitB.lsl` (one per sitter slot)

`MENU_LIST` (pose names from LSD), `ANIM_INDEX`, menu state, settings cache. **Does not hold** `DATA_LIST`/`POS_ROT_LIST` — those are read on demand from LSD via `qs_pose_data(idx)`. This is the key memory-saving change vs stock.
...
Creator-tool runtime state only (helper-bar mode, listen channel, sitter tracking, menu navigation). No persistent state of its own — LSD writes go through `qs_save_pose_offset` / `qs_add_pose` into the shared `qs:p:<ch>:<i>` namespace.
